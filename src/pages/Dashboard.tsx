...
export function Dashboard() {
  ...
  // Internal match stats (Team A vs Team B)
  const internalMatchStats = useMemo(() => {
    ...
    const teamAWins = completedInternal.filter(m => m.winning_team === 'team_a').length;
    const teamBWins = completedInternal.filter(m => m.winning_team === 'team_b').length;
  ...
  const animatedTeamAWins = useAnimatedValue(internalMatchStats.teamAWins, 800);
  const animatedTeamBWins = useAnimatedValue(internalMatchStats.teamBWins, 800);
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 via-white to-primary-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 min-h-screen">
      <Header title="Dashboard" subtitle={`Welcome to ${club?.name || 'Your Club'}`} />
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 text-white">
                  <h3 className="text-lg font-bold">Want to Join {club?.name || 'Our Club'}?</h3>
          ...
          <Card delay={370} className="overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                Internal Matches - {club?.team_a_name || 'Team A'} vs {club?.team_b_name || 'Team B'}
              </h3>
            </div>
            <CardContent className="p-6">
              <div className="flex flex-col lg:flex-row items-center justify-center gap-8">
                ...
                <div className="flex-1 text-center">
                  ...
                  <h4 className="text-lg font-bold text-blue-600 dark:text-blue-400">{club?.team_a_name || 'Team A'}</h4>
                ...
                <div className="flex-1 text-center">
                  ...
                  <h4 className="text-lg font-bold text-purple-600 dark:text-purple-400">{club?.team_b_name || 'Team B'}</h4>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Wins</p>
                </div>
              </div>
              ...
              <div className="mt-6">
                <div className="h-3 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden flex">
                  {internalMatchStats.teamAWins > 0 && (
                    <div
                      className="h-full bg-gradient-to-r from-blue-400 to-blue-500 transition-all duration-1000"
                      style={{
                        width: `${(internalMatchStats.teamAWins / internalMatchStats.total) * 100}%`,
                      }}
                    ></div>
                  )}
                  {internalMatchStats.draws > 0 && (
                  ...
                  {internalMatchStats.teamBWins > 0 && (
                    <div
                      className="h-full bg-gradient-to-r from-purple-400 to-purple-500 transition-all duration-1000"
                      style={{
                        width: `${(internalMatchStats.teamBWins / internalMatchStats.total) * 100}%`,
                      }}
                    ></div>
                  )}
                </div>
                <div className="flex justify-between mt-2 text-xs text-gray-500">
                  <span className="text-blue-500 font-medium">{club?.team_a_name || 'Team A'}</span>
                  <span className="text-purple-500 font-medium">{club?.team_b_name || 'Team B'}</span>
                </div>
              </div>
            </CardContent>
          </Card>

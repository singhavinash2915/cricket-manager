...
export function WhatsAppRemindersModal({
  ...
  const { club } = useClub();
  const paymentLink = club?.payment_link || '';
  const clubName = club?.name || 'Cricket Club';
  ...
  const getMessage = (member: Member) => {
    ...
    let message =
      `Hi ${member.name}, your ${clubName} balance is ₹${member.balance.toFixed(0)}. ` +
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
          ...
          <p className="text-sm text-green-700 dark:text-green-300">
            ...
            balance below ₹{threshold} can be notified.{paymentLink ? ' Message includes the payment link.' : ''}
          </p>
        </div>

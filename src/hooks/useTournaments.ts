...
export function useTournaments() {
        ...
        .select(`
          *,
          matches:tournament_matches(
            id, tournament_id, match_id, stage,
            match:matches(*)
          )
        `)
        .eq('club_id', club.id)
  ...
  const addTournament = async (tournament: Omit<Tournament, 'id' | 'created_at' | 'matches' | 'club_id'>) => {
    if (!club) throw new Error('No club selected');
    try {
      const { data, error } = await supabase
        .from('tournaments')
        .insert([{ ...tournament, club_id: club.id }])
        .select()
        .single();
  ...
  const addMatchToTournament = async (tournamentId: string, matchId: string, stage: TournamentMatch['stage']) => {
    try {
      const { error } = await supabase
        .from('tournament_matches')
        .insert([{ tournament_id: tournamentId, match_id: matchId, stage }]);
  ...
  const removeMatchFromTournament = async (tournamentId: string, matchId: string) => {
    try {
      const { error } = await supabase
        .from('tournament_matches')
        .delete()
  ...
  const getTournamentStats = (tournament: Tournament) => {
    ...
    const completedMatches = matches.filter(m => m.match && ['won', 'lost', 'draw'].includes(m.match.result));
  ...
  return {
    tournaments, loading, error, fetchTournaments,
    addTournament, updateTournament, deleteTournament,
    addMatchToTournament, removeMatchFromTournament, getTournamentStats,
  };
}
